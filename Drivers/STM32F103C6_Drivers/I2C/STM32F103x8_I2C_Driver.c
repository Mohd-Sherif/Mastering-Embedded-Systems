...
void MCAL_I2C_Init(I2C_Typedef *I2Cx, I2C_Config_t *I2C_Config){
	...
	if(I2C_Config->mode == I2C_MODE_I2C_MODE)
	{
		...
		if((I2C_Config->clockSpeed == I2C_CLOCK_SPEED_50KHZ)||(I2C_Config->clockSpeed == I2C_CLOCK_SPEED_100KHZ))
		{
		...
		Temp_Register |= (uint16_t)(I2C_Config->ACK_Control | I2C_Config->generalCall | I2C_Config->mode | I2C_Config->stretchMode);
		...
		if(I2C_Config->slaveAddress.Enable_Dual_Address == 1)
		{
			 ...
			 */
			Temp_Register |= I2C_Config->slaveAddress.Slave_Second_Address << I2C_OAR2_ADD2_Pos; // Shifting with the first bit in the register
		...
		Temp_Register |= I2C_Config->slaveAddress.Slave_First_Address << 1 ;

		/* 6. Configure ADDMODE Addressing mode (slave mode) */
		Temp_Register |= I2C_Config->slaveAddress.Slave_Address_Mode ;
...
void MCAL_I2C_GPIO_Set_Pins(I2C_Typedef *I2Cx){
	...
	if(I2Cx == I2C1){
		/* PB6 : I2C1_SCL */
		I2C_GPIO_Config.pinNumber = GPIO_PIN_6;
		I2C_GPIO_Config.mode = GPIO_MODE_OUTPUT_AF_OD;
		I2C_GPIO_Config.outputSpeed = GPIO_SPEED_10M;
		MCAL_GPIO_Init(GPIOB, &I2C_GPIO_Config);

		/* PB7 : I2C1_SDA */
		I2C_GPIO_Config.pinNumber = GPIO_PIN_7;
		I2C_GPIO_Config.mode = GPIO_MODE_OUTPUT_AF_OD;
		I2C_GPIO_Config.outputSpeed = GPIO_SPEED_10M;
		MCAL_GPIO_Init(GPIOB, &I2C_GPIO_Config);
	}
		...
		/* PB10 : I2C2_SCL */
		I2C_GPIO_Config.pinNumber = GPIO_PIN_10;
		I2C_GPIO_Config.mode = GPIO_MODE_OUTPUT_AF_OD;
		I2C_GPIO_Config.outputSpeed = GPIO_SPEED_10M;
		MCAL_GPIO_Init(GPIOB, &I2C_GPIO_Config);

		/* PB10 : I2C2_SDA */
		I2C_GPIO_Config.pinNumber = GPIO_PIN_11;
		I2C_GPIO_Config.mode = GPIO_MODE_OUTPUT_AF_OD;
		I2C_GPIO_Config.outputSpeed = GPIO_SPEED_10M;
		MCAL_GPIO_Init(GPIOB, &I2C_GPIO_Config);
	}
